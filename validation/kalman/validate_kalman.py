...
def run_validation():
    ...
    lines.append("Used synthetic AR(1) autocorrelated data. Trained on sparse subset (every 10th day) using Standard WRTDS, then applied Kalman correction (rho=0.9) to predict test days.")
    lines.append("")

    lines.append("### Python Implementation Snippet")
    lines.append("```python")
    lines.append("# Initialize Decanter with combined data (Test rows are NaN)")
    lines.append("dec_combined = Decanter(df_combined, 'Date', 'Conc', 'Q')")
    lines.append("dec_combined.decant_series(h_params, use_grid=True)")
    lines.append("")
    lines.append("# Get Standard Predictions")
    lines.append("est_log = dec_combined.get_estimated_series(h_params, use_grid=True)")
    lines.append("")
    lines.append("# Apply Kalman Correction")
    lines.append("kalman_log = dec_combined.add_kalman_correction(est_log, rho=0.9)")
    lines.append("```")
    lines.append("")
