...
def run_validation():
    ...
    report_lines.append("This report validates the Python `Decanter` implementation of Standard WRTDS against the established R `EGRET` package.")
    report_lines.append("The validation uses a synthetic daily dataset with known Discharge and Concentration series.")
    report_lines.append("")
    ...
    report_lines.append("1. **Data Loading:**")
    report_lines.append(f"   - Daily Records: {len(df_daily)}")
    report_lines.append(f"   - Calibration Samples: {len(df_sample)}")
    report_lines.append("2. **Model Training:**")
    report_lines.append("   - Python: `Decanter.decant_series(h_params, use_grid=True)`")
    report_lines.append("   - R: `modelEstimation(eList)` (Pre-computed in `egret_results.csv`)")
    report_lines.append("3. **Comparison:**")
    report_lines.append("   - RMSE and Percentage RMSE (PRMSE) are calculated for both Estimated Concentration (Daily) and Flow Normalized Concentration.")
    report_lines.append("")
    report_lines.append("### Python Implementation Snippet")
    report_lines.append("```python")
    report_lines.append("# Initialize Decanter with Sample and Daily History")
    report_lines.append("dec = Decanter(df_sample, 'Date', 'Conc', 'Q', daily_data=df_daily)")
    report_lines.append("")
    report_lines.append("# Fit Grid and Predict")
    report_lines.append("h_params = {'h_time': 7, 'h_cov': 2, 'h_season': 0.5}")
    report_lines.append("dec.decant_series(h_params, use_grid=True)")
    report_lines.append("results = dec.predict(df_daily, use_grid=True)")
    report_lines.append("```")
    report_lines.append("")
