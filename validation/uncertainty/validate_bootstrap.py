...
def run_validation():
    ...
    lines.append("Ran Block and Wild Bootstrap (n=10) on synthetic data to generate confidence intervals (5th-95th percentile).")
    lines.append("")
    lines.append("### Python Implementation Snippet")
    lines.append("```python")
    lines.append("# Run Bootstrap (Block method)")
    lines.append("res_block = dec.bootstrap_uncertainty(h_params, n_bootstraps=10, method='block', use_grid=True)")
    lines.append("")
    lines.append("# Run Bootstrap (Wild method)")
    lines.append("res_wild = dec.bootstrap_uncertainty(h_params, n_bootstraps=10, method='wild', use_grid=True)")
    lines.append("```")
    lines.append("")
    lines.append("### R Reproduction Code")
    lines.append("```r")
    lines.append("# To verify against EGRETci:")
    lines.append("# library(EGRETci)")
    lines.append("# ciLower <- ciCalculations(eList, ..., method='block')")
    lines.append("```")
    lines.append("")
