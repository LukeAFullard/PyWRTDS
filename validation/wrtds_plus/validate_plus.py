...
def run_validation():
    ...
    lines.append("WRTDSplus allows the inclusion of additional covariates (e.g., Temperature, Groundwater Level) into the regression model.")
    lines.append("This validation checks if adding a known driver (Temperature) reduces prediction error compared to the standard Q-only model.")
    lines.append("")
    lines.append("## Methodology")
    lines.append("Synthetic data generated with Conc ~ Q + Temp. Models trained on 20% of data, tested on 80%.")
    lines.append("")
    lines.append("### Python Implementation Snippet")
    lines.append("```python")
    lines.append("# Configure Extra Covariate")
    lines.append("extra_cov = [{'col': 'Temp', 'log': False}]")
    lines.append("dec_plus = Decanter(df_train, 'Date', 'Conc', 'Q', extra_covariates=extra_cov)")
    lines.append("")
    lines.append("# Set window width for Temp")
    lines.append("h_params_plus = {'h_time': 7, 'h_cov': 2, 'h_season': 0.5, 'h_Temp': 5}")
    lines.append("")
    lines.append("# Train Grid (N-Dimensional) and Predict")
    lines.append("dec_plus.decant_series(h_params_plus, use_grid=True)")
    lines.append("res_plus = dec_plus.predict(df_test, use_grid=True)")
    lines.append("```")
    lines.append("")
    lines.append("**Note:** This feature is a Python-specific extension. No direct comparison with standard R EGRET is available.")
    lines.append("")
